...
CHENTRY=$(echo -e "$TOGGLE\n$LIST" | uniq -u | rofi -dmenu -selected-row 1 -config "~/.config/scripts/wifi_menu/theme.rasi")
# store selected SSID
...
if [ "$CHENTRY" = "" ]; then
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "" ]]; then
            # separate password config to allow a prompt
			WIFIPASS=$(echo " Press Enter if network is saved" | rofi -dmenu -p " Password for $CHSSID: " -lines 1 -config "~/.config/scripts/wifi_menu/password.rasi")
		fi
        # throws out cancels or dud inputs
        if [[ ! $WIFIPASS == "" ]]; then
